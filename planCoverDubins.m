...
function path=planCoverDubins(wpp_start, R, map)
    ...
    for i=1:SEARCH_CYCLES
        tree = extendTree(path(end,:),L,vartheta,depth,map,return_map,pd, segmentLength);
...
%%   point
function tree = extendTree(start_node,L,vartheta,depth,map,return_map,pd, segmentLength)
...
%     pe0 = path(i,2);
%     for j = 1:50
%         pn = pn0 + j*(path(i+1,1)-pn0)/50;
%         pe = pe0 + j*(path(i+1,2)-pe0)/50;
%         [pn_max,pe_max] = size(return_map);
%         fn = pn_max*pn/map.width;
%         fn = min(pn_max,round(fn));
%         fn = max(1,fn);
